...
exports.convert = async (type, extension) => {
  ...
  if (notecount > 1) {
    ...
    notifyCompletedMultiple(notecount , type)
  } else if (notecount === 1) {
    ...
    await new Promise(resolved => pandoc("markdown", extension).stream(Readable.from([note.body])).pipe(createWriteStream(filePath)).on("finish", resolved))
    notifyCompleted(note.title, type)
  } else {
    notifyNoNote()
  }
}
...
const fs = require('fs')
exports.convertBook = async (event, type, extension) => {
  const { bookList: { bookForContextMenu } } = inkdrop.store.getState()
  const bookId = (event.detail || {}).bookId || (bookForContextMenu || {})._id
  if (bookId) {
    exportNotesInBook(bookId, type, extension)
  } else {
    notifyNoBook()
  }
}

async function exportNotesInBook(bookId, type, extension) {
  ...
  try {
    await exportBook(pathToSave, book, { createBookDir: false }, type, extension)
...
async function exportBook(parentDir, book, opts = {}, type, extension) {
  ...
  for (let i = 0; i < notes.length; ++i) {
    ...
    await new Promise(resolved => pandoc("markdown", extension).stream(Readable.from([notes[i].body])).pipe(createWriteStream(savePath)).on("finish", resolved))
  }

  if (book.children) {
    await book.children.reduce((promise, childBook) => {
      return promise.then(() => exportBook(pathToSave, childBook, {}, type, extension))
    }, Promise.resolve())
  }
}
