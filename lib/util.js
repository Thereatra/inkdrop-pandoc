  ...
  if (notecount > 1) {
    ...
    for (let noteId of noteIds) {
      ...
      if (note) {
        ...
        await new Promise(resolved => pandoc("markdown", doctype).stream(Readable.from([note.body])).pipe(createWriteStream(filePath)).on("finish", resolved))
      }
    }
...
async function exportBook(parentDir, book, opts = {}, type, extension, doctype) {
  ...
  for (let i = 0; i < notes.length; ++i) {
    ...
    await new Promise(resolved => pandoc("markdown", doctype).stream(Readable.from([notes[i].body])).pipe(createWriteStream(savePath)).on("finish", resolved))
  }
